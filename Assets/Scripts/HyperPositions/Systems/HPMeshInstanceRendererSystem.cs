...
/// <summary>
/// Renders all Entities containing both HPMeshInstanceRenderer & LocalToWorld components.
/// </summary>
[ExecuteInEditMode]
public class HPMeshInstanceRendererSystem : ComponentSystem
{
    ...
    protected override void OnCreateManager()
    {
        m_FrozenChunksQuery = GetComponentGroup(new EntityArchetypeQuery
        {
            ...
            All = new ComponentType[] { typeof(LocalToWorld), typeof(HPMeshInstanceRenderer), typeof(VisibleLocalToWorld), typeof(Frozen) }
        });
        m_DynamicChunksQuery = GetComponentGroup(new EntityArchetypeQuery
        {
            Any = Array.Empty<ComponentType>(),
            None = new ComponentType[] { typeof(Frozen) },
            All = new ComponentType[] { typeof(LocalToWorld), typeof(HPMeshInstanceRenderer), typeof(VisibleLocalToWorld) }
        });
        ...
        GetComponentGroup(new EntityArchetypeQuery
        {
            ...
            All = new ComponentType[] { typeof(HPMeshInstanceRenderer), typeof(LocalToWorld) }
        });
    ...
    {
        [ReadOnly] public NativeArray<ArchetypeChunk> Chunks;
        [ReadOnly] public ArchetypeChunkSharedComponentType<HPMeshInstanceRenderer> HPMeshInstanceRendererType;
        public NativeMultiHashMap<int, int>.Concurrent ChunkRendererMap;

        public void Execute(int index)
        {
            var chunk = Chunks[index];
            var rendererSharedComponentIndex = chunk.GetSharedComponentIndex(HPMeshInstanceRendererType);
    ...
    unsafe void UpdateFrozenInstanceRenderer()
    {
        ...
        var HPMeshInstanceRendererType = GetArchetypeChunkSharedComponentType<HPMeshInstanceRenderer>();
        ...
        for (int i = 0; i < packedChunkCount; i++)
        {
            ...
            var rendererIndex = chunk.GetSharedComponentIndex(HPMeshInstanceRendererType);
    ...
    void UpdateFrozenChunkCache()
    {
        ...
        {
            Chunks = chunks,
            HPMeshInstanceRendererType = HPMeshInstanceRendererType,
